...
*S8 · Subagent reframe: drop the "profile" concept — **DONE** (this PR).* `AgentRole` replaces `AgentProfile` (`src/agent/subagents.ts` `AgentRole` primary, `AgentProfile` deprecated alias; `BUILTIN_AGENT_ROLES` single read-only `explorer`, `BUILTIN_AGENT_PROFILES` alias; `loadAgentRoles`/`formatAgentRolesForSystemPrompt` primary with legacy aliases). `agentsFolder`/`enableBuiltinAgents` kept but `@deprecated` (`src/settings-schema.ts`); vault `AGENT.md` still loads with a `console.warn` for backward compat. All import sites use `AgentRole`; `model` override kept for H5 evaluator routing. Settings UI + README + `docs/reference/settings.md` note the deprecation.
