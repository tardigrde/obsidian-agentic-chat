...
*S8 · Subagent reframe: drop the "profile" concept — **DONE** (this PR).* `AgentRole` replaces `AgentProfile` (`src/agent/subagents.ts` `AgentRole` primary, `AgentProfile` deprecated alias; `BUILTIN_AGENT_ROLES` single read-only `explorer`, `BUILTIN_AGENT_PROFILES` deep-copy alias; `loadAgentRoles`/`formatAgentRolesForSystemPrompt` primary with legacy aliases). `agentsFolder`/`enableBuiltinAgents` kept but `@deprecated` with heal coercion (`src/settings-schema.ts`); vault `AGENT.md` still loads with a once-per-session `console.warn` for backward compat (override-by-name is case-insensitive). Runtime types use `AgentRole` (`getProfiles` field/method names kept as compat); `model` override kept for H5 evaluator routing. Settings UI + README + `docs/reference/settings.md` note the deprecation.
...
- **Problem**: Evaluator subagent shares same model as generator; no separate eval arm. Harness #53-54 `deviates` / #64. Vault edits not independently graded.
- **Goal**: Optional evaluator routing: `/deep-research` adversarial review pass + any future verifier role can target cheaper/stronger model via `AgentRole.model` override, distinct from generator model. (S8 done: single built-in `explorer` covers both search fan-out and adversarial review; `model` override preserved.)
- **Approach**: Keep current explorer-as-verifier prompt; `model` field use in `subagent-runtime.ts:69` already supports fallback. Add settings-level `reviewerModel` or reuse per-role `model` + documentation. Long-term: pipeline `planner→generator→evaluator` as skill, not harness core.
- **Files**: `src/agent/subagents.ts` (`EXPLORER_PROMPT`), `src/agent/subagent-runtime.ts`, `docs/` (`/deep-research` skill)
- **Acceptance**: Search→Review→Synthesis run uses two different model ids when the verifier role specifies `model`; synthesis cites review findings severity-ordered.
- **Effort**: S
