...
**UI rendering.** `ChatView` (`src/ui/chat-view.ts`) renders both the static transcript and live pi events. `AssistantBubble` owns the DOM of one assistant turn (reasoning `<details>`, tool step cards, streamed text, usage footer). Slash commands are parsed in `handleSlashCommand`; informational ones (`/help`, `/status`, `/usage`) render as a collapsible in-pane block via `renderInfoMessage` and are **never sent to the model**. The model picker uses `SuggestModal` (not `FuzzySuggestModal`) so filtered results stay alphabetical.
