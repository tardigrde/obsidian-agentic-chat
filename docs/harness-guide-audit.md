...
| 18 | Network isolation / egress allowlist | `settings.web.allowedHosts` positive allowlist (`src/tools/web-allowlist.ts` / `src/tools/web-fetch.ts`) sits on top of existing layers: (a) master egress gate (`web-tools.ts` — tools not registered when off), (b) `isBlockedHost` SSRF deny-list. Allowlist is label-boundary-aware (`example.com` → `sub.example.com` ok, `evil-example.com` blocked), supports `*.` / `.` prefix and `*` wildcard, checked after `isBlockedHost` (deny wins) and on every redirect hop via `fetchFollowingRedirects`. Empty = allow all public. UI normalizes on save (`src/settings.ts`). | compliant | H1 closed: positive egress control ships in #110; `noProxy` remains proxy-bypass only (distinct from destination allowlist) |
| 19 | Input sanitization for untrusted content (web/MCP) | Web/MCP/vault tool outputs are wrapped with `[BEGIN_UNTRUSTED_TOOL_OUTPUT ...]` / `[END_UNTRUSTED_TOOL_OUTPUT]` markers via `src/tools/tool-output-wrapper.ts` (`wrapToolOutput` + `escapeToolOutput` for inner `BEGIN`/`END` marker sequences, `unwrapToolOutput` for tests); `src/tools/vault-tools.ts` `textResult`/`untrustedTextResult`, `src/mcp/tools.ts` `renderMcpResult` (incl. `structuredContent` stringified path), `src/tools/web-fetch.ts` and `src/tools/web-search.ts` all wrap. System prompt states marker content is untrusted DATA, never instructions. `redactValue` in `privacy/redaction.ts` remains at audit/observability edge. Injection payload text (`Ignore previous instructions`, `</TOOL>`, fake `tool_call` JSON) remains as data inside the wrapper; only inner `BEGIN`/`END` marker sequences are escaped. | compliant | H7 closed: explicit wrapper + escaping + prompt boundary ships in #111; not a security boundary — authorization stays in `tool-call-controller.ts` |
...
| 35 | Per-subagent timeout | `settings.subagentTimeoutSeconds` enforced via `AbortController` + `setTimeout(healSubagentTimeout)` in `src/tools/subagent-tool.ts:271`, surfaced as `Timed out after Ns` / `aborted` not `error`; `0` disables, max `86400` `src/settings-schema.ts:271` | compliant | H4 closed: wall-clock timeout ships; spend cap remains as cost backstop |
...
- **Closed via H1 #110: per-user `fetch_url` destination allowlist (row 18).** `settings.web.allowedHosts` positive allowlist (`src/tools/web-allowlist.ts`) now sits atop the `isBlockedHost` SSRF floor, validated on every hop via `fetchFollowingRedirects`.
