...
| 19 | Input sanitization for untrusted content (web/MCP) | Web/MCP/vault tool outputs are wrapped with `[BEGIN_UNTRUSTED_TOOL_OUTPUT ...]` / `[END_UNTRUSTED_TOOL_OUTPUT]` markers via `src/tools/tool-output-wrapper.ts` (`wrapToolOutput` + `escapeToolOutput` for inner `BEGIN`/`END`, `unwrapToolOutput` for tests); `src/tools/vault-tools.ts:880` `textResult`, `src/mcp/tools.ts` `renderMcpResult` (incl. `structuredContent` stringified path), `src/tools/web-fetch.ts` and `src/tools/web-search.ts` all wrap. System prompt states marker content is untrusted DATA, never instructions. `redactValue` in `privacy/redaction.ts` remains at audit/observability edge. Injection payloads (`Ignore previous instructions` + `</TOOL>` + fake `tool_call` JSON + inner markers) are escaped inside the wrapper. | compliant | H7 closed: explicit wrapper + escaping + prompt boundary ships in #111; not a security boundary — authorization stays in `tool-call-controller.ts` |
