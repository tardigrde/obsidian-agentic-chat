...
- **Dispatch.** The agent calls the `subagent` tool strictly one call per child (`{ agent, task }`); to delegate several tasks it makes several `subagent` calls in one message, which run concurrently (capped in flight). Each child runs with its own context window, the profile's model (falling back to the parent's), and **only** the tools in its allowlist — so a research fan-out that can't write never prompts you.
