...
- **Local data lifecycle** — `/export` saves the active conversation as Markdown, `/sessions clear --confirm` clears conversation history for the current scope, and `/memory add` + `/memory distill` manage Tier-1 daily notes and the distilled MEMORY.md (see `docs/features/memory.md`).
...
- **Roles.** A subagent is a role — `{ name, description, systemPrompt, model?, toolAllowlist }`. The single built-in **Explorer** role (read-only recon, inherits parent approval/mode) is the whole roster; custom roles are not supported — extend the roster in code.
