...
2. **Dispatch parallel searchers.** Make several parallel \`subagent\` calls, one per \`researcher\` child, each \`{agent: "researcher", task: "<sub-question>"}\`, to run 2-6 researchers at once. Give each child one focused sub-question and explicitly tell it to use \`web_search\` and \`fetch_url\`, fetch promising results before relying on them, and return claim-by-claim evidence with source artifact ids and URLs.
